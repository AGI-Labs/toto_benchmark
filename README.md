...
To evaluate an agent following the BC training provided in the example train.py script, run the following command:
  ```
python toto_benchmark/sim/eval_agent.py -f outputs/<path_to>/<agent>/ -n <your_team_name>
  ```
If you wish to evaluate a custom agent, replace the agent_predict_fn indicated by the TODO(optional) in toto_benchmark/sim/eval_agent.py.

To submit your agent, the following items need to be prepared: 
- The `txt` file generated by running the eval script `toto_benchmark/sim/eval_agent.py` on your agent.
- An `environment.yml` file specifying your environment setup. Here is an example command to generate the file:
  
  ```
  conda env export > environment.yml
  ```
  Save this environment file in the top level of your code directory so that it is in the submitted code zip (the next item).
- A zip file for launching `toto_benchmark/sim/eval_agent.py` for your agent. Your code folder should contain `eval_agent.py` and any other files needed to launch your agent. Here is an example command to zip your code folder:
  ```
  zip -r your_team_name.zip <directory name> 
  ```
  If your agent is built in the toto_benchmark repository, the following example command excludes dataset files to make the zip smaller:
  ```
  zip -r ../example_submission.zip <path to toto_benchmark> -x assets/ -x toto_benchmark/sim/dm_human_dataset.pickle
  ```
