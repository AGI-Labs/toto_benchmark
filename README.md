...
- After your model is trained, update the file `vision/CollaboratorEncoder.py` for loading your model, as well as any transforms needed.
    ...
    - Please put your vision model outside of the `assets/` folder, as it will be ignored when generating a `.zip` file for submission later.
...
- Now we are ready to train a BC agent! We will be using the config file `train_bc.yaml`. Update `vision_model_path` in `train_bc.yaml` if needed.
